...
- 流式模式自动添加简单标点：
  ...
  - 其他句子末尾自动加"。"
  - 已有标点的句子不重复添加
- 流式模式长停顿段落分隔：两句之间停顿超过 3 秒自动插入空行
...
- 新增悬浮窗显示转录内容，可在其他应用上方实时查看
  - 支持拖动移动位置
  - 底部拖动条可调节窗口大小
  - 透明度滑块可调（10% ~ 100%）
  - 位置、大小、透明度自动保存
- 转录文本自动保存到本地文件 `/sdcard/AudioListener/日期.txt`，带时间戳
- 新增 Notion 上传功能：将当天转录文件上传为 Notion 子页面
  - 配置方法：将 API Key 存入 `/sdcard/AudioListener/notion_api_key.txt`，页面 ID 存入 `notion_page_id.txt`
